...
from core.ham import hmc_sampler,pmmc_sampler
...
def log_q(state, mean, log_var):
    ...
    return -0.5*var.shape[1].value*tf.log(2*np.pi)-0.5* tf.reduce_sum(log_var,1) + res
...
def train(setting, dataset, dataset_name='mnist', save_model=False, device='CPU', dtype=tf.float32):
    ...
    model_name = "vae_{}-hei-{}-alpha{:.0e}-zd{}-hd{}-mbs{}-mbn{}-h{}-l{}-reg{:.0e}-{}-lr{:.0e}".format(generator,
                                                                                              ...
                                                                                              num_lfsteps,
                                                                                              reg, bin_label, lr)
    output_dir = "model/mnist_ahmc-vi/"
    ...
    with tf.device(device_config):
        ...
        state_final, avg_acp = pmmc_sampler(pot_fun_train, state_init, num_samples=num_layers, burn_in=0, num_leaps=5,s=0.6, step_size=step, dtype=tf.float32)
