...
def train(setting, dataset, dataset_name='mnist', save_model=False, device='CPU', dtype=tf.float32):
    ...
    model_name = "vae_{}-hei-{}-alpha{:.0e}-zd{}-hd{}-mbs{}-mbn{}-h{}-l{}-reg{:.0e}-{}-lr{:.0e}".format(generator,
                                                                                              ...
                                                                                              num_lfsteps,
                                                                                              reg, bin_label, lr)
    output_dir = "model/fmnist_ahmc-kl_a0/"
    ...
    with tf.device(device_config):
        ...
        vaeq = VAEQ_CONV(alpha = alpha, h_dim=h_dim, z_dim=z_dim)  # 编码器
        vae = VAE_DCNN_GPU(h_dim=h_dim, z_dim=z_dim)  # 解码器
        ...
        hamInfNet_hm = HamInfNet(num_layers=num_layers, num_lfsteps=num_lfsteps,
                                 sample_dim=z_dim, s=0.6, dtype=dtype)
        ...
        # do not train decoder(trained by max E-log-target) params using alpha_divergence

        loss =  pot_batch_mean + q_loss + reg * (vae.get_parameters_reg() + vaeq.get_parameters_reg())
        scale = tf.Variable(initial_value=1., trainable=False, dtype=tf.float32)
    ...
    with tf.Session() as sess:
        ...
        for i in np.arange(0,100000):  # pretrain encoder+decoder using stadard VAE/IWAE training scheme
            ...
            start=time.time()
            _,q_loss1_i,inflation_i=sess.run([train_op2,qloss1,inflation],feed_dict={X_batch_train:X_mb})
...
if __name__ == '__main__':
    ...
    train(setting=training_setting(32), dataset=mnist, save_model=True, device="cpu") # 32
