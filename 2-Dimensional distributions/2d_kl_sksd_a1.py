...
def generate_samples_HMC(params, n = 100):  
    ...
    for j in range(L):
        v = rs.randn(n, params.shape[ 1 ]) * np.exp(0.5 * log_v_r[ j, : ])
        z_new, v_new = leapfrog(z, v, np.exp(log_eps[ j, : ]), log_v_r[ j, : ], dlogP)
        p_acceptance = np.minimum(1, np.exp(logP(z_new) - logP(z) -0.5 * np.sum(v_new**2 /  np.exp(log_v_r[ j, : ]), 1) + \
            0.5 * np.sum(v**2 /  np.exp(log_v_r[ j, : ]), 1)))
...
def evaluate_objective(params): 
    ...
    log_w = (logP(samples0) - (-0.5 *(((samples0[:,0]-getval(mu0[0]))**2)/getval(var0[0]) + \
        ((samples0[:,1]-getval(mu0[1]))**2)/getval(var0[1])) -\
                          np.log(2*np.pi) -0.5 * np.log(getval(var0[0]*var0[1]))))
    
    alpha_div = np.sum(getval(np.exp(log_w - logsumexp(log_w))**2.0) * log_w)
    ...
    return -np.mean(logP(samples_HMC)) -alpha_div + max_SKSD
