...
def train(setting, dataset, dataset_name='mnist', save_model=False, device='CPU', dtype=tf.float32):
    ...
    model_name = "vae_{}-hei-{}-alpha{:.0e}-zd{}-hd{}-mbs{}-mbn{}-h{}-l{}-reg{:.0e}-{}-lr{:.0e}".format(generator,
                                                                                              ...
                                                                                              num_lfsteps,
                                                                                              reg, bin_label, lr)
    output_dir = "model/fmnist_ahmc-kl_a1/"
    ...
    with tf.device(device_config):
        ...
        q_loss = vaeq.create_loss_not_train(vae, X_batch_train, batch_size = 5, loss_only=True) # do not train decoder(trained by max E-log-target) params using alpha_divergence
        ...
        qloss1=vaeq.create_loss_train(vae,X_batch_train,batch_size=5,loss_only=True) # pretrain decoder params along with encoder params using standard VAE/IWAE scheme
