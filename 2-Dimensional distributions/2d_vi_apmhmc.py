...
def leapfrog(z, v,  log_v_r, dlogP, eps=0.01):
    for i in range(5):
...
def generate_samples(eps0,params,n=100):
    ...
    z0 = z
    acp = np.zeros(n)
    v0 = rs.randn(n, params.shape[1])
    r = np.ones((n, 2)) * 0.1  # 噪声

    for j in range(L):
        mu = rs.randn(n, params.shape[1])
        v2 = np.sqrt(1 - r) * v0 + np.sqrt(r) * mu
        mu2 = -np.sqrt(r) * v0 + np.sqrt(1 - r) * mu
        r_acceptance = np.minimum(1, np.exp(- 0.5 * np.sum(v2 ** 2 / np.exp(0), 1) + \
                                            0.5 * np.sum(v0 ** 2 / np.exp(0), 1) - \
                                            0.5 * np.sum(mu2 ** 2 / np.exp(0), 1) + \
                                            0.5 * np.sum(mu ** 2 / np.exp(0), 1)))
        r_accepted = rs.rand(r.shape[0]) < r_acceptance
        r_accepted_tile = np.transpose(np.tile(r_accepted, (params.shape[1], 1)))
        v0 = v2 * r_accepted_tile + (1 - r_accepted_tile) * v0

        z_new, v_new = leapfrog(z, v0, 0, dlogP, eps)
        p_acceptance = np.minimum(1, np.exp(logP(z_new) - logP(z) - 0.5 * np.sum(v_new ** 2 / np.exp(0), 1) + \
                                            0.5 * np.sum(v0 ** 2 / np.exp(0), 1)))
        ...
        acp += accepted
        r_acp = np.transpose(np.tile(p_acceptance, (params.shape[1], 1)))

        r = np.ones((n, 2)) * 0.2 + 0.8 * (1 - r_acp)
...
def generate_samples_z0(eps0,z0,params,n=10):
    ...
    sigma0 = np.exp(log_sigma0)
    eps = sigma0*eps0
    v0 = rs.randn(z.shape[0], z0.shape[1])
    r = np.ones((z.shape[0], z0.shape[1])) * 0.1  # 噪声

    for j in range(L):
        mu = rs.randn(z.shape[0], z0.shape[1])
        v2 = np.sqrt(1 - r) * v0 + np.sqrt(r) * mu
        mu2 = -np.sqrt(r) * v0 + np.sqrt(1 - r) * mu
        r_acceptance = np.minimum(1, np.exp(- 0.5 * np.sum(v2 ** 2 / np.exp(0), 1) + \
                                            0.5 * np.sum(v0 ** 2 / np.exp(0), 1) - \
...
def generate_samples_apmhmc(eps0, params, n=100):
    mu0 = params[0, :]
    mu0 = np.ones((n, 1)) * mu0
    log_sigma0 = params[1, :]
    sigma0 = np.exp(log_sigma0)
    ...
    for j in range(L):
        ...
        r = np.ones((n, 2)) * 0.1 + 0.9 * (0.9 - r_acp)
...
if __name__ == '__main__':
    ...
    z,_,_ = generate_samples_apmhmc(eps0,params, 100000)
