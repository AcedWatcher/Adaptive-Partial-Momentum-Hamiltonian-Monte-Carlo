...
from data import load_iwae_binarised_mnist_dataset,load_mnist_binary_test,load_fashion_mnist_binary_test
...
def demo(dataset, device="GPU", dtype=tf.float32):
    ...
    with tf.device(device_config):
        ...
        log_partition, log_weights, sample, acp_rate = hais_gauss(pot_target=pot_fun, num_chains=100, input_batch_size=mb_demo_size, dim=z_dim,
                                                              ...
                                                              num_leaps=5,
                                                              step_size=0.2)#100,1000
        

    saver = tf.train.Saver()
    
    with tf.Session() as sess:
        saver.restore(sess, model_path+"vae_dcnn_relu-hei-mnist-alpha0e+00-zd32-hd500-mbs128-mbn95000-h30-l5-reg1e-06-dybin-lr2e-04.ckpt-50000")
...
if __name__ == '__main__':
    ...
    saved = demo(dataset=fmnist,device='GPU')
    np.savetxt('fmnist_.csv',saved.reshape(10000),delimiter=',')
