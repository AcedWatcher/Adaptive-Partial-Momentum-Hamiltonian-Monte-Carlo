...
def leapfrog(z, v, eps, log_v_r, dlogP):
    for i in range(5):
        v_half = v - eps / 2.0 * -getval(dlogP(z))
        z = z + eps * v_half / np.exp(log_v_r)
        v = v_half - eps / 2.0 * -getval(dlogP(z))
    return z, v
...
def generate_samples_HMC(eps0, params, n = 100):  
    ...
    acp = np.zeros(n)
    for j in range(L):
        v = rs.randn(n, params.shape[ 1 ]) * np.exp(0.5 * 0)
        z_new, v_new = leapfrog(z, v, eps, 0.0, dlogP)
        z_acceptance = np.minimum(1, np.exp(logP(z_new) - logP(z) -0.5 * np.sum(v_new**2 /  np.exp(0), 1) + \
            0.5 * np.sum(v**2 /  np.exp(0), 1)))
        accepted = rs.rand(n) < z_acceptance  # n-dimensional, ie. 100-dimensional
