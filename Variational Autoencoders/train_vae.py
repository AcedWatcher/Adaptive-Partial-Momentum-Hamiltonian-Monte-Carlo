...
def train(setting, dataset, dataset_name='mnist', save_model=False, device='CPU', dtype=tf.float32):
    ...
    with tf.device(device_config):
        ...
        q_loss = vaeq.create_loss_train(vae, X_batch_train, batch_size = 1, loss_only=True) # do not train decoder(trained by max E-log-target) params using alpha_divergence
