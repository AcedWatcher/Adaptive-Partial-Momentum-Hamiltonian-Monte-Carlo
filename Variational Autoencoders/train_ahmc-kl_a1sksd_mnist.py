...
def train(setting, dataset, dataset_name='mnist', save_model=False, device='CPU', dtype=tf.float32):
    ...
    output_dir = "model/mnist_ahmc-kl_a1sksd/"
    ...
    with tf.device(device_config):
        ...
        def gen_fun_train_maxsksd(sample_batch_size, input_data_batch_size ,inflation):
            mu, log_var = vaeq.Q(X_batch_train)
            mu_nograd = tf.stop_gradient(mu)
            log_var_nograd = tf.stop_gradient(log_var)
            eps = tf.random_normal(shape=(sample_batch_size, input_data_batch_size, z_dim))
        ...
        maxsksd_mean = hamInfNet_hm.build_maxsksd_graph(
            ...
            state_init_gen=gen_fun_train_maxsksd,
            input_data_batch_size=mb_size,
        ...
        q_loss1 = vaeq.create_loss_train(vae, X_batch_train, batch_size = 5, loss_only=True)
        
        
        q_loss2 = vaeq.create_loss_not_train(vae, X_batch_train, batch_size = 5,loss_only=True)
    ...
    with tf.Session() as sess:
        ...
        total_time = 0
        for i in np.arange(0, 100000):   # pretrain encoder+decoder using standard VAE/IWAE training scheme
