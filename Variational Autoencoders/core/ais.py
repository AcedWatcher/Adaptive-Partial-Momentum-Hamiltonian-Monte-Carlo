...
def hais(pot_target, pot_init, sample_init, log_init_partition, schedule, num_leaps=5, step_size=0.1,
    ...
    #sample_init_shape = tf.shape(sample_init)
    #log_weights_init = tf.zeros(shape=(100,100), dtype=dtype)  # num_chains * input_batch      dim: num_chains x pot_dim

    log_weights_init = tf.zeros(shape=(sample_init.shape[0], sample_init.shape[1]), dtype=dtype)  # num_chains * input_batch      dim: num_chains x pot_dim
